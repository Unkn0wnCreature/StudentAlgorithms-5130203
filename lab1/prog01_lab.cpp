...
// **************************************************************
	...
	iInt	=	257;
	cByte	=	iInt; // поскольку на char выделяется 8 бит, то при вводе 257, в переменную записывается остаток от деления на 256 (кольцо вычетов)

	unsigned char cN1=255, cN2=2, cSum;
	cSum = cN1 + cN2; // значение в переменной cSum = 257, но поскольку char имеет только 8 битов, то в переменную записывается остаток от деления на 256 (кольцо вычетов)
	...
	// Напишите явно преобразования, которые неявно выполняет компилятор
	int iSum = cN1+cN2; // компилятор приводит сумму к высшему формату. В данном случае к int. На int выделяется 4 байта, поэтому значение не выходит за разрядную сетку
	...
	int iSum1= c1 + c2;   //(1) компилятор приводит сумму к высшему формату. В данном случае к int. (iSum1 = (int)c1 + int(c2) = (-128 + 127) + 2 = (-1) + 2 = 1)
	int iSum2= uc1 + uc2; //(2) компилятор приводит сумму к высшему формату. В данном случае к int. (iSum = (int)uc1 + (int)uc2 = 255 + 2 = 257)
